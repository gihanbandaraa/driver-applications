...
import { Text, ScrollView, TouchableOpacity, Image, Linking, Dimensions, Alert, ActivityIndicator} from 'react-native';
...
const StudentDetails = () => {
        ...
        const handleDeleteStudent = async () => {
            ...
            try {
                ...
                // Remove the deleted student from AsyncStorage
                const storedStudents = await AsyncStorage.getItem('students');
                if (storedStudents) {
                    const students = JSON.parse(storedStudents);
                    const updatedStudents = students.filter((s: Student) =>
                        s.id !== parseInt(studentId as string)
                    );
            ...
            <SafeAreaView className="flex-1 bg-primary-900">
                ...
                <ScrollView className="flex-1">
                    ...
                    <View className="bg-gray-50 rounded-t-3xl -mt-4 px-5 pt-6 min-h-screen">
                        ...
                        <TouchableOpacity
                            className={`${isDeleting ? 'bg-gray-300' : 'border-2 border-red-500'} p-4 rounded-xl mb-8`}
                            onPress={() => {
                                if (!isDeleting) {
                                    Alert.alert(
                                        'Delete Confirmation',
                        ...
                        >
                            {isDeleting ? (
                                <View className="flex-row justify-center items-center">
                                    ...
                                    <Text className="text-gray-600 text-center font-JakartaBold text-lg ml-2">
                                        Deleting...
                                    </Text>
                                </View>
                            ) : (
                                <Text className="text-red-500 text-center font-JakartaBold text-lg">
                                    Delete Student
                                </Text>
                            )}
                        </TouchableOpacity>
